...
function RequestCard({ req, currentUser, currentUserId, isAdmin, done, rejected }: {
  ...
  const hasVoted = req.upvotedBy.includes(currentUserId);
  ...
  return (
    <div className={`bg-neutral-900/60 border ${borderColor} rounded-lg p-4`} id={`req-${req.$id}`}>
      <div className="flex gap-3">
        {/* Upvote */}
        {!done && !rejected && (
          <form method="post" action="/v1/feature-requests/upvote" className="flex-shrink-0">
            <input type="hidden" name="id" value={req.$id} />
            <button type="submit" className={`flex flex-col items-center px-2 py-1 rounded transition-colors ${
              hasVoted ? 'bg-purple-600/30 text-purple-300' : 'bg-neutral-800 text-neutral-400 hover:bg-neutral-700 hover:text-white'
            }`}>
